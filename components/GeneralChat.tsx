...
export default function GeneralChat() {
  ...
  useEffect(() => {
    ...
    const savedSessionId = localStorage.getItem('healthmateSessionId');
    if (savedSessionId) {
    ...
    const storedHistory = localStorage.getItem('healthmateChatHistory');
    if (storedHistory) {
      try {
  ...
  useEffect(() => {
    if (chatHistory.length > 0) {
      localStorage.setItem('healthmateChatHistory', JSON.stringify(chatHistory));
    }
  }, [chatHistory]);
  ...
  useEffect(() => {
    if (sessionId) {
      localStorage.setItem('healthmateSessionId', sessionId);
    }
  }, [sessionId]);
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('healthmateSessionId');
    localStorage.removeItem('healthmateChatHistory');
