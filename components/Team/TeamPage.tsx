      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="card-title flex justify-between items-end text-[#ffffffa9]">
          ...
          <div className="flex gap-2">
            <Link  href={data.social.linkedIn} target='_main'><Linkedin size={18} strokeWidth={1.25} /></Link>
            <Link  href={data.social.twitter} target='_main'><Twitter size={18} strokeWidth={1.25} /></Link>
          </div>
        </div>
      </div>
    </div>
...
const CompanyReview = ({ data }) => {
  return (
    <div className="company-review col-lg-6 col-12 p-2 px-10 mb-[5rem]">
...
const TeamModel = ({ isOpen, onClose, member }) => {
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full bg-[#000000af] z-[100] flex justify-center items-center">
      <div className="bg-[#141414] max-w-[90%] h-[75vh] lg:h-[80vh] w-[85%] md:w-[50%] lg:w-[30%] rounded-4xl flex flex-col">
        ...
        {/* middle section */}
        <div className="h-[85%] max-h-[85%] overflow-y-scroll">
          {/* Name */}
...
const TeamPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setTeam(teamRes.data.filter(m => m.status === 1));
      setReviews(reviewRes.data.filter(r => r.status === 1));
    };
    fetchData();
  }, []);
      ...
      <div>
        ...
        <div className="row w-full justify-between">
          <div className="flex flex-col text-3xl mb-3">
            <span>The People Behind the</span>
