...
import { Search, Users, Award, ArrowLeft } from "lucide-react";
...
export default function CertificatePage() {
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      try {
        ...
        const res = await axios.get(`${base}/certificate`);
  ...
  const filteredMembers = members.filter((member) => {
    ...
    const matchesCategory =
      selectedCategory === "all" || member.category?.toLowerCase() === selectedCategory.toLowerCase();
    return matchesSearch && matchesCategory;
  });

  const handleCardClick = (memberId) => {
    router.push(`/certificate/${memberId}`);
  };
  ...
  if (loading) {
    return (
      <ThemeProvider attribute="class" defaultTheme="dark">
        <main className="relative min-h-screen bg-gradient-to-b from-black to-gray-900 text-white overflow-hidden">
          <NoiseTexture />
          <ParticlesBackground />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredMembers.map((member, index) => (
                  ...
                  >
                    ...
                    >
                      <div className="bg-gray-800/30 backdrop-blur-sm border border-gray-700/30 rounded-xl p-6 hover:bg-gray-700/30 transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-blue-500/10 relative">
                            ...
                            <span
                              className={`px-2 py-1 text-xs rounded-full ${
                                member.category?.toLowerCase() === "leadership"
                                  ...
                                  : member.category?.toLowerCase() === "employee"
